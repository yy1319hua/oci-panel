...
func SetPushplusToken(token string) error {
	...
	db := database.GetDB()
	setting := models.SysSetting{Key: SettingPushplusToken, Value: token}
	result := db.Where("`key` = ?", SettingPushplusToken).Assign(models.SysSetting{Value: token}).FirstOrCreate(&setting)
	if result.Error != nil {
		return result.Error
	}
	if token == "" {
		return db.Model(&models.SysSetting{}).Where("`key` = ?", SettingPushplusToken).Update("value", "").Error
	}
	return nil
}
